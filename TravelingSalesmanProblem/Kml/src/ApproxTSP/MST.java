...
public class MST {
	...
	public static void creatKML(Graph graph, ArrayList<Integer> tsp , ArrayList<Integer> optimal) {
		ArrayList<Double> optimalX = new ArrayList<Double>();
		ArrayList<Double> optimalY = new ArrayList<Double>();
		ArrayList<Double> tspX = new ArrayList<Double>();
		ArrayList<Double> tspY = new ArrayList<Double>();
		for (int i = 0; i < tsp.size();i++) {
			String [] crimeRecord = graph.getRecord(tsp.get(i)).split(",");
			tspX.add(Double.parseDouble(crimeRecord[8]));
			tspY.add(Double.parseDouble(crimeRecord[7]));
		}
		for(int j = 0; j < optimal.size(); j++) {
			String [] optimalRecord = graph.getRecord(optimal.get(j)).split(",");
			optimalX.add(Double.parseDouble(optimalRecord[8])+0.0005);
			optimalY.add(Double.parseDouble(optimalRecord[7])+0.0005);
		}
		try {
			File newFile = new File("PGHCrimes.kml");
			newFile.createNewFile();
			FileWriter newKML = new FileWriter(newFile);
			newKML.write("<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\n"+
			...
			"<coordinates>\n");
			for(int i = 0; i< tspX.size();i++) {
				newKML.write(tspX.get(i) + "," +tspY.get(i)+","+ 0.000000+"\n");
			}
			newKML.write("</coordinates>\n"+			
					"</LineString>\n"+
					"</Placemark>\n"+
					"<Placemark>\n"+
			...
			for(int j = 0; j< optimalX.size(); j++) {
				newKML.write(optimalX.get(j)+","+optimalY.get(j)+","+0.000000+"\n");
			}
			newKML.write("</coordinates>\n"+
			       "</LineString>\n"+
					"</Placemark>\n"+
...
//		System.out.println(optimalcycle);
		...
		creatKML(graph,tsp,optimalPath);
